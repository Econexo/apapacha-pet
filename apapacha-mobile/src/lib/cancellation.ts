...
// Tarifas no reembolsables (deben coincidir con el checkout y el RPC cancel_booking).
export const APP_FEE = 4500;
export const INSURANCE_FEE = 2500;
const NON_REFUNDABLE = APP_FEE + INSURANCE_FEE;
...
export function refundPreview(opts: {
  ...
  const { startDate, totalPrice, paid, byHost } = opts;
  ...
  const refundable = paid ? Math.max(0, totalPrice - NON_REFUNDABLE) : 0;
